...
`RecordWriter<T>`インターフェースの共通シグネチャを以下の通り確定する（Application Design Q4「共通インターフェース+Strategyパターン」を実装可能な形に具体化）:
```java
public interface RecordWriter<T> {
    void write(List<T> records, Class<T> type, Path outputPath, Charset charset, boolean append) throws IOException;
}
```
`type`（`Class<T>`）を渡すことで、`CsvRecordWriter`はレコード0件（BR-7）でも対象の型に応じたヘッダーを解決できる。CSV/TSVはJava 25のパターンマッチング`switch`で型ごとのヘッダー・行変換ロジックを内部に持つ。JSON/YAML用Writerは`type`引数を使用しない（Jacksonが対象インスタンスから直接シリアライズするため）。

## Steps

- [ ] **Step 1: build.gradle 依存関係更新**
  - 対象: `build.gradle`（修正）
...
- [ ] **Step 5: Jackson設定生成**
  ...
  - 内容: `@Configuration`。`YAMLMapper`の`@Bean`定義。

- [ ] **Step 6: オーケストレーション層改修**
