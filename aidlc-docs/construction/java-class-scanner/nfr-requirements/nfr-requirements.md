...
### NFR-R1: 集約書き込みのメモリ使用量方針（改訂: 全フォーマットに適用範囲を拡大）
Functional DesignのBR-8/BR-9改訂により、CSV/TSVを含む全フォーマットが全レコードをメモリ上に蓄積してから1回で書き出す方式に統一された。大規模スキャン時のメモリ使用量について、現時点では特別な対策（上限設定・ストリーミング書き込み等）を設けない。当初はJSON/YAMLのみを対象にこの判断を行っていたが、CSV/TSVの逐次書き込み（ストリーミング）という既存の省メモリ特性を手放してでもコードのシンプルさを優先する、というユーザーの明示的判断により、全フォーマットに適用範囲を拡大した。本ツールの想定利用規模ではメモリ使用量は問題にならないと判断する。将来的に問題が顕在化した場合は別途対応する。
